  ...
  useEffect(() => {
    ...
    const obs = new IntersectionObserver(([e]) => {
      if (e.isIntersecting && !started.current) {
        ...
        let cur = 0;
        const step = () => {
          cur += Math.ceil(value / 18);
          if (cur >= value) { setDisplay(value); return; }
          setDisplay(cur); requestAnimationFrame(step);
        };
...
function FAQ({ q, a }: { q: string; a: string }) {
  ...
  return (
    <div style={{ borderBottom: "1px solid var(--v-pearl)" }}>
      <button onClick={() => setOpen(!open)} className="hover:!text-[#a0845c]" style={{ display: "flex", justifyContent: "space-between", alignItems: "center", width: "100%", padding: "22px 0", background: "none", border: "none", fontFamily: "var(--v-body)", fontSize: 14, color: "var(--v-charcoal)", cursor: "pointer", textAlign: "left" as const, gap: 16, fontWeight: 400, letterSpacing: ".01em", transition: "color .3s" }}>
        {q}
...
function galleryLabel(i: number, type: string): string {
  const labels: Record<string, string[]> = { bar: ["Craft Cocktails", "The Bar", "The Atmosphere"], lounge: ["The Lounge", "Drinks", "The Vibe"], spa: ["Treatments", "The Spa", "Relaxation"], museum: ["Exhibits", "The Gallery", "The Experience"] };
  return (labels[type] || ["View 1", "View 2", "View 3"])[i] || `View ${i + 1}`;
}
...
export default function VenuePageContent({ venue, relatedVenues = [] }: { venue: VenueProps; relatedVenues?: RelatedVenue[] }) {
  ...
  const cssVars = {
    "--v-white": "#ffffff", "--v-snow": "#fdfcfb", "--v-bone": "#f5f2ee", "--v-pearl": "#e8e4de", "--v-sand": "#d4cfc6", "--v-stone": "#8a857c", "--v-charcoal": "#3a3832", "--v-ink": "#1a1917",
  ...
  return (
    <div style={{ ...cssVars, fontFamily: "var(--v-body)", background: "var(--v-white)", color: "var(--v-ink)", overflowX: "hidden" }} onMouseMove={handleMouse}>

      {/* Cursor glow */}
      {isDesktop && <div style={{ position: "fixed", top: 0, left: 0, width: 320, height: 320, borderRadius: "50%", background: "radial-gradient(circle, rgba(160,132,92,.07) 0%, transparent 70%)", pointerEvents: "none", zIndex: 9998, transform: `translate(${mousePos.x - 160}px, ${mousePos.y - 160}px)`, transition: "transform .08s linear" }} />}

      {/* ═══ 1. HERO ═══ */}
      <section style={{ position: "relative", height: "100vh", minHeight: 700, overflow: "hidden", background: "#0a0908" }}>
        <div style={{ position: "absolute", inset: 0, overflow: "hidden" }}>
          {venue.heroImage && <Image src={venue.heroImage} alt={venue.name} fill priority sizes="100vw" style={{ objectFit: "cover", opacity: 0, transform: "scale(1.12)", animation: "venueHeroReveal 2.2s cubic-bezier(.23,1,.32,1) .3s forwards", filter: "brightness(.42) saturate(1.1)" }} />}
        </div>
        {/* Grain */}
        ...
        <div style={{ position: "absolute", top: "clamp(28px,5vh,56px)", left: "clamp(32px,7vw,120px)", right: "clamp(32px,7vw,120px)", display: "flex", justifyContent: "space-between", alignItems: "center", zIndex: 4, opacity: 0, animation: "venueFadeDown .8s ease .8s forwards" }}>
          ...
          <div style={{ display: "flex", gap: 28 }} className="max-sm:hidden">
            <Link href="/experiences-things-to-do-in-reno-nv/" className="hover:!text-white/60" style={{ fontSize: 10, color: "rgba(255,255,255,.25)", letterSpacing: 2, textTransform: "uppercase", transition: "color .3s" }}>Experiences</Link>
            <Link href="/contact-custom-golf-package/" className="hover:!text-white/60" style={{ fontSize: 10, color: "rgba(255,255,255,.25)", letterSpacing: 2, textTransform: "uppercase", transition: "color .3s" }}>Book</Link>
          </div>
        </div>
        ...
        <div style={{ position: "absolute", inset: 0, zIndex: 3, display: "flex", flexDirection: "column", justifyContent: "flex-end", padding: "0 clamp(32px,7vw,120px) clamp(56px,9vh,110px)" }}>
          ...
          <h1 style={{ fontFamily: "var(--v-serif)", fontWeight: 300, fontSize: "clamp(52px,9vw,120px)", lineHeight: .9, color: "#fff", letterSpacing: "-.03em", opacity: 0, animation: "venueTextUp 1.2s cubic-bezier(.23,1,.32,1) 1.1s forwards" }}>
      ...
      <section style={{ display: "grid", gridTemplateColumns: "1fr 1fr", minHeight: 650 }} className="max-md:!grid-cols-1">
        <div style={{ padding: "clamp(56px,9vh,110px) clamp(36px,6vw,90px)", display: "flex", flexDirection: "column", justifyContent: "center", background: "var(--v-white)", position: "relative" }}>
          ...
          <R delay={0.32}><div style={{ width: 48, height: 1, background: "var(--v-pearl)", margin: "32px 0" }} /></R>
          <R delay={0.4}><p style={{ fontSize: 14, lineHeight: 1.95, color: "var(--v-stone)", fontWeight: 300, maxWidth: 480 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gridTemplateRows: "1fr 1fr", gap: 4 }} className="max-md:!min-h-[400px]">
          {gallery.slice(0, 3).map((img, i) => (
            <R key={i} mode="scale" delay={0.15 + i * 0.15} style={{ overflow: "hidden", position: "relative", cursor: "pointer", ...(i === 2 ? { gridColumn: "span 2" } : {}) }}>
              <div style={{ position: "relative", width: "100%", height: "100%" }} onClick={() => setLbIndex(i)} className="group">
                <Image src={img} alt={`${venue.name} ${i+1}`} fill className="object-cover brightness-[.85] saturate-[1.05] group-hover:brightness-100 group-hover:saturate-[1.1] group-hover:scale-[1.06] transition-all duration-700" sizes="(max-width:900px) 100vw, 50vw" />
                <div className="absolute inset-0 bg-gradient-to-t from-black/30 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500" />
                <div className="absolute bottom-3.5 left-4 text-[9px] tracking-[2px] uppercase text-white/70 font-medium opacity-0 translate-y-2 group-hover:opacity-100 group-hover:translate-y-0 transition-all duration-500 delay-100">{galleryLabel(i, venue.type)}</div>
              </div>
            </R>
          ))}
          {gallery.length === 0 && <div style={{ gridColumn: "span 2", gridRow: "span 2", background: "var(--v-bone)", display: "flex", alignItems: "center", justifyContent: "center" }}><span style={{ fontSize: 11, color: "var(--v-stone)", letterSpacing: 2, textTransform: "uppercase" }}>Photos Coming Soon</span></div>}
        </div>
      </section>
      ...
      <section style={{ padding: "clamp(64px,10vh,120px) clamp(36px,7vw,120px)", background: "var(--v-bone)", position: "relative", overflow: "hidden" }}>
        ...
        <R delay={0.1}><h2 style={{ fontFamily: "var(--v-serif)", fontWeight: 300, fontSize: "clamp(30px,3.5vw,52px)", lineHeight: 1.08, marginBottom: 40 }}>What to <em style={{ fontStyle: "italic" }}>Know</em></h2></R>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 18 }} className="max-md:!grid-cols-2 max-sm:!grid-cols-1">
          {(venue.highlights || []).map((h, i) => (
            <R key={i} delay={0.15 + i * 0.07}>
              <div className="group hover:-translate-y-1 hover:shadow-[0_20px_50px_rgba(0,0,0,.06)] hover:!border-transparent" style={{ padding: "28px 24px", background: "var(--v-white)", borderRadius: 20, border: "1px solid var(--v-pearl)", position: "relative", overflow: "hidden", transition: "all .5s cubic-bezier(.23,1,.32,1)", cursor: "default" }}>
                <div className="group-hover:!scale-x-100" style={{ position: "absolute", top: 0, left: 0, right: 0, height: 3, background: "linear-gradient(90deg, var(--v-bronze), var(--v-bronze-glow))", transform: "scaleX(0)", transformOrigin: "left", transition: "transform .5s cubic-bezier(.23,1,.32,1)" }} />
                <span className="group-hover:scale-[1.15] group-hover:-rotate-[5deg]" style={{ fontSize: 28, display: "block", marginBottom: 12, transition: "transform .4s" }}>{h.icon}</span>
      ...
      <section style={{ display: "grid", gridTemplateColumns: "1fr 1fr", background: "var(--v-ink)", position: "relative" }} className="max-md:!grid-cols-1">
        <div style={{ position: "relative", overflow: "hidden", minHeight: 500 }} className="max-md:!min-h-[300px] group">
          {gallery[0] && <Image src={gallery[0]} alt={venue.name} fill className="object-cover opacity-40 saturate-[.8] group-hover:opacity-[.55] group-hover:scale-[1.05] transition-all duration-[12s]" sizes="(max-width:900px) 100vw, 50vw" />}
        ...
        <div style={{ padding: "clamp(56px,9vh,110px) clamp(36px,6vw,90px)", display: "flex", flexDirection: "column", justifyContent: "center" }}>
          ...
          <div style={{ marginTop: 32 }}>
            {(venue.insiderTips || []).map((tip, i) => {
              ...
              return (
                <R key={i} delay={0.2 + i * 0.08}>
                  <div className="group hover:pl-2" style={{ display: "flex", gap: 18, padding: "18px 0", borderBottom: i < (venue.insiderTips?.length || 0) - 1 ? "1px solid rgba(255,255,255,.04)" : "none", transition: "all .4s", cursor: "default" }}>
                    <div className="group-hover:!bg-[#a0845c] group-hover:!text-white group-hover:!border-[#a0845c]" style={{ width: 32, height: 32, borderRadius: "50%", border: "1px solid rgba(160,132,92,.25)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 12, color: "var(--v-bronze)", flexShrink: 0, fontWeight: 500, transition: "all .4s" }}>{i + 1}</div>
                    <div>
                      {parts.length > 1 ? (<><div style={{ fontSize: 14, color: "rgba(255,255,255,.7)", fontWeight: 500, marginBottom: 4 }}>{parts[0]}</div><div style={{ fontSize: 12, color: "rgba(255,255,255,.28)", fontWeight: 300, lineHeight: 1.65 }}>{parts.slice(1).join(" — ")}</div></>) : (<div style={{ fontSize: 13, color: "rgba(255,255,255,.5)", fontWeight: 300, lineHeight: 1.65 }}>{tip}</div>)}
                    </div>
                  </div>
                </R>
              );
      ...
      <section style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 0 }} className="max-md:!grid-cols-1">
        <div style={{ padding: "clamp(56px,9vh,96px) clamp(36px,6vw,90px)", background: "var(--v-bone)" }}>
          ...
          {(venue.distances || []).length > 0 && (
            <R delay={0.18}><div style={{ marginTop: 28 }}>
              {(venue.distances || []).slice(0, 6).map((d, i) => (
                <div key={i} className="hover:pl-1.5 transition-all duration-300" style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "14px 0", borderBottom: "1px solid var(--v-pearl)", fontSize: 13, fontWeight: 300 }}>
          ...
          {addr?.streetAddress && (
            <R delay={0.28}>
              <div className="hover:shadow-[0_12px_36px_rgba(0,0,0,.04)] transition-all duration-400" style={{ marginTop: 28, padding: 24, background: "var(--v-white)", borderRadius: 20, border: "1px solid var(--v-pearl)" }}>
                ...
                <div style={{ display: "flex", gap: 10, marginTop: 16, flexWrap: "wrap" }}>
                  {venue.directionsUrl && <a href={venue.directionsUrl} target="_blank" rel="noopener noreferrer" className="hover:-translate-y-0.5 hover:shadow-[0_8px_28px_rgba(160,132,92,.2)]" style={{ display: "inline-flex", alignItems: "center", padding: "10px 24px", borderRadius: 100, background: "var(--v-bronze)", color: "#fff", fontSize: 10, fontWeight: 500, letterSpacing: 1.5, textTransform: "uppercase", textDecoration: "none", transition: "all .4s" }}>Directions</a>}
                  {venue.website && <a href={venue.website} target="_blank" rel="noopener noreferrer" className="hover:!border-[#a0845c] hover:!text-[#a0845c] hover:-translate-y-0.5" style={{ display: "inline-flex", alignItems: "center", padding: "10px 24px", borderRadius: 100, border: "1px solid var(--v-pearl)", color: "var(--v-charcoal)", fontSize: 10, fontWeight: 500, letterSpacing: 1.5, textTransform: "uppercase", textDecoration: "none", transition: "all .4s" }}>Website</a>}
                </div>
              </div>
            </R>
          )}
        ...
        <div style={{ padding: "clamp(56px,9vh,96px) clamp(36px,6vw,90px)", background: "var(--v-white)" }}>
          ...
          {venue.groupEvents && (
            <R delay={0.48}>
              <div className="hover:shadow-[0_8px_24px_rgba(0,0,0,.03)] transition-all duration-400" style={{ marginTop: 32, padding: 28, background: "var(--v-bone)", borderRadius: 24, border: "1px solid var(--v-pearl)" }}>
                ...
                {venue.groupEvents.reservationUrl && <a href={venue.groupEvents.reservationUrl} target="_blank" rel="noopener noreferrer" className="hover:!bg-[#a0845c] hover:!text-white hover:-translate-y-0.5" style={{ display: "inline-flex", marginTop: 16, padding: "9px 22px", borderRadius: 100, border: "1px solid var(--v-bronze)", color: "var(--v-bronze)", background: "none", fontSize: 10, fontWeight: 500, letterSpacing: 1.5, textTransform: "uppercase", textDecoration: "none", transition: "all .4s" }}>Reserve</a>}
              </div>
            </R>
          )}
        </div>
      ...
      {relatedVenues.length > 0 && (
        <section style={{ padding: "clamp(48px,7vh,80px) clamp(36px,7vw,120px)", background: "var(--v-bone)" }}>
          ...
          <R delay={0.1}><h2 style={{ fontFamily: "var(--v-serif)", fontWeight: 300, fontSize: "clamp(30px,3.5vw,52px)", lineHeight: 1.08 }}>Nearby <em style={{ fontStyle: "italic" }}>Experiences</em></h2></R>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 18, marginTop: 28 }} className="max-md:!grid-cols-1">
            {relatedVenues.map((rv, i) => (
              <R key={rv.slug} delay={0.14 + i * 0.06}>
                <Link href={`/portfolio/${rv.slug}/`}>
                  <div style={{ borderRadius: 20, overflow: "hidden", border: "1px solid var(--v-pearl)", background: "var(--v-white)", transition: "all .5s cubic-bezier(.23,1,.32,1)" }} className="hover:!border-transparent hover:!shadow-[0_20px_50px_rgba(0,0,0,.06)] hover:-translate-y-1">
                    <div style={{ aspectRatio: "16/9", overflow: "hidden", position: "relative" }}>
                      {rv.heroImage ? <Image src={rv.heroImage} alt={rv.name} fill className="object-cover brightness-[.88] hover:brightness-100 hover:scale-[1.05] transition-all duration-700" sizes="(max-width:768px) 100vw, 33vw" /> : <div style={{ width: "100%", height: "100%", background: "var(--v-pearl)" }} />}
                      <span style={{ position: "absolute", top: 12, left: 12, background: "rgba(0,0,0,.6)", backdropFilter: "blur(8px)", color: "#fff", padding: "4px 14px", borderRadius: 100, fontSize: 9, fontWeight: 500, letterSpacing: 1, textTransform: "uppercase" }}>{typeLabel(rv.type)}</span>
                    </div>
      ...
      <section style={{ background: "var(--v-ink)", textAlign: "center", padding: "clamp(80px,12vh,140px) clamp(36px,7vw,120px)", position: "relative", overflow: "hidden" }}>
        ...
        <div style={{ position: "relative", zIndex: 1 }}>
          ...
          <R delay={0.4}><div style={{ display: "flex", gap: 12, justifyContent: "center", flexWrap: "wrap" }}>
            <Link href="/contact-custom-golf-package/" className="hover:-translate-y-[3px] hover:shadow-[0_16px_48px_rgba(160,132,92,.25)] hover:!bg-[#b89b6e]" style={{ display: "inline-block", padding: "16px 40px", background: "var(--v-bronze)", color: "#fff", borderRadius: 100, fontSize: 10, fontWeight: 600, letterSpacing: 2.5, textTransform: "uppercase", transition: "all .5s cubic-bezier(.23,1,.32,1)" }}>Plan My Trip</Link>
            <a href="tel:+18885848232" className="hover:!border-white/30 hover:!text-white" style={{ display: "inline-block", padding: "16px 40px", border: "1px solid rgba(255,255,255,.08)", color: "rgba(255,255,255,.3)", borderRadius: 100, fontSize: 10, fontWeight: 400, letterSpacing: 2.5, textTransform: "uppercase", transition: "all .4s" }}>Call 888-584-8232</a>
          </div></R>
        </div>
      </section>
      ...
      <style jsx global>{`
        ...
        @keyframes venueTextUp{from{opacity:0;transform:translateY(50px)}to{opacity:1;transform:translateY(0)}}
